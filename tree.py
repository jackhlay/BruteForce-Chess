...
class Node:
    ...
    def UCT(self, parentVisited: int | None = 0, c: float = math.sqrt(2)) -> float:
        if self.visited == 0:
            return float("inf")
...
def select_with_path(root: Node):
    ...
    while current.children:
        current = max(
            ...
            key=lambda n: n.UCT(parentVisited=max(current.visited,1)),
        )
        path.append(current)
    return current, path
...
def rollout(board: chess.Board, max_deptht: int = 19) -> float:
    ...
    while not board.is_game_over() and depth < max_deptht:
        ...
        #
        caps_checks = [
            m for m in moves if (board.is_capture(m) or board.gives_check(m))
        ]
...
def backProp_Path(path, valueFromWhite: float):
    for node in reversed(path):
        signed = (
            valueFromWhite if node.gameState.turn == chess.WHITE else -valueFromWhite
        )
